		...
		int v)
{
	...
	if (!scene3d.isInit())
	{
		...
		scene3d.setCurrentFrame(581);
		scene3d.processFrame();
		scene3d.getReconstructor().update();
		scene3d.getReconstructor().initialiseCluster(4, 0);

		//im = scene3d.getCameras()[0]->getFrame();
		//imshow("test1", im);
		//waitKey(0);
		...
		scene3d.processFrame();
		scene3d.getReconstructor().update();
		scene3d.getReconstructor().initialiseCluster(4, 1);

		//im = scene3d.getCameras()[1]->getFrame();
		//imshow("test", im);
		//waitKey(0);

		scene3d.setCurrentFrame(914);
		scene3d.processFrame();
		scene3d.getReconstructor().update();
		scene3d.getReconstructor().initialiseCluster(4, 2);

		//im = scene3d.getCameras()[2]->getFrame();
		//imshow("test", im);
		//waitKey(0);

		scene3d.setCurrentFrame(515);
		scene3d.processFrame();
		scene3d.getReconstructor().update();
		scene3d.getReconstructor().initialiseCluster(4, 3);

		//im = scene3d.getCameras()[3]->getFrame();
		//imshow("test", im);
		//waitKey(0);

		scene3d.getReconstructor().clearTrackingVoxels();
		scene3d.setInit(true);
	...
	if (scene3d.isPaused())
	{
		//scene3d.getReconstructor().printFlickerCount(); // Used for debugging/validating purposes.
	}
	if (!scene3d.isPaused())
	{
	...
	if (scene3d.getCurrentFrame() != scene3d.getPreviousFrame())
	{
		...
		scene3d.getReconstructor().updateClusters(4);
...
void Glut::drawVoxels()
{
	...
	{
		//glColor4f(0.5f, 0.5f, 0.5f, 0.5f);
		glColor4f(voxels[v]->color.val[2], voxels[v]->color.val[1], voxels[v]->color.val[0], 0.5f);
		glVertex3f((GLfloat) voxels[v]->x, (GLfloat) voxels[v]->y, (GLfloat) voxels[v]->z);
	}

	vector<Reconstructor::Voxel*> t_voxels = m_Glut->getScene3d().getReconstructor().getTrackingVoxels();
	for (size_t v = 0; v < t_voxels.size(); v++)
	{
		//glColor4f(0.5f, 0.5f, 0.5f, 0.5f);
		glColor4f(t_voxels[v]->color.val[2], t_voxels[v]->color.val[1], t_voxels[v]->color.val[0], 0.5f);
		glVertex3f((GLfloat)t_voxels[v]->x, (GLfloat)t_voxels[v]->y, (GLfloat)t_voxels[v]->z);
	}
